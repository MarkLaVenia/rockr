...
Execution of these scripts requires the installation and loading of the following packages:
...
Data entry excludeed polls conducted for:
...
Note that I use the # temporailly disable code. I do this so that this same chunk of code can serve all three analysis, enabling/disabling different scripts depending on the analysis. I also use the # at the end of strings of code that are analysis dependent to cue when and which to enable/disable.

Also observe that I do not save `formatted` files for each analysis. So, these scripts are structured to move directly from creating the `formatted` file to rendering the bar chart before creating a `formatted` file for anaother analysis.
...
#bands_count <- length(unique(analysisfile_formatted$band_name)) # length of list of bands for which colors are needed
#bands_count

#bands <- unique(analysisfile_formatted$band_name) # generate list of bands for which colors are needed
#bands
